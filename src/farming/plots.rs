...
pub fn spawn_crop_plots(
    ...
    for ty in min_ty..max_ty {
        for tx in min_tx..max_tx {
            ...
            let stage = restored_stage(saved, tx, ty);
            spawn_one_plot(commands, art, tx, ty, center, stage);
        }
    }
}
...
pub fn advance_all_plots_on_sleep(plots: &mut Query<&mut CropPlot>) {
...
fn plant_visual(stage: PlotStage) -> LayerVisual {
    match stage {
        ...
        PlotStage::Growing {
            ...
            days,
            watered,
        } => growing_plant_visual(crop, days, watered),
        PlotStage::Ready { crop } => ready_plant_visual(crop),
    }
}

fn growing_plant_visual(crop: CropKind, days: u8, watered: bool) -> LayerVisual {
    if days == 0 {
        let tint = match crop {
            CropKind::Turnip if watered => Color::srgb(0.85, 0.7, 0.95),
            CropKind::Turnip => Color::srgb(0.95, 0.85, 1.0),
            CropKind::Potato if watered => Color::srgb(0.95, 0.85, 0.55),
            CropKind::Potato => Color::srgb(1.0, 0.95, 0.75),
        };
        return LayerVisual {
            color: tint,
