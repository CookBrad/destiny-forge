...
use super::pause::{world_unpaused, WorldPause};
...
impl Plugin for CorePlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(MemoryPlugin)
            ...
            .init_resource::<WorldPause>()
            .init_state::<GameState>()
            .add_sub_state::<DungeonPlayState>()
            .add_systems(
                Update,
                tick_day_clock
                    .run_if(in_state(GameState::Overworld).or(in_state(GameState::Forest)))
                    .run_if(world_unpaused),
            )
            .add_systems(
