...
- GitHub Actions runs `cargo test --locked --bin destiny_forge` with `RUSTFLAGS=-D warnings -A dead_code` on every PR and on `develop`. Unused imports/mut fail. Unused pub APIs (`dead_code`) are allowed until wired. CI does not replace the local run.
